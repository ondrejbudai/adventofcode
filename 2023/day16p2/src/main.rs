...
fn sim_beam(map: &HashMap<(i32, i32), Tile>, beam: Beam) -> usize{
...
fn main() {
    ...
    // yolo
    let size = ((map.len() as f64).sqrt() as i32);

    for i in 0..size {
        max = max.max(sim_beam(&map, Beam{x: 0, y: i, dir: Direction::Right}));
        max = max.max(sim_beam(&map, Beam{x: size-1, y: i, dir: Direction::Left}));
        max = max.max(sim_beam(&map, Beam{x: i, y: 0, dir: Direction::Down}));
        max = max.max(sim_beam(&map, Beam{x: i, y: size-1, dir: Direction::Up}));
    }
